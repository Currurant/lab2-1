...
```{r}
objects = senators$objects
for (i in 1:100){
  if (str_detect(objects[[i]]$description, "North Carolina")){
    print(objects[[i]]$person$birthday)
  }
}
```
...
```{r}
for (i in 1:100){
  if (!is.null(objects[[i]]$leadership_title)){
    print(objects[[i]]$person$twitterid)
  }
}
```
